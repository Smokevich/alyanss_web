...
public function getMethod($address, $total) {
    ...
    if ($this->config->get('payment_yandexplusplus_card_status')) {
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('payment_yandexplusplus_card_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

			if (!$this->config->get('payment_yandexplusplus_card_geo_zone_id')) {
				$status = TRUE;
				if ($this->config->get('payment_yandexplusplus_card_maxpay')){
						if ($this->currency->format($total, 'RUB', $this->currency->getValue('RUB'), false) <= $this->config->get('payment_yandexplusplus_card_maxpay')) {
							$status = true;
							if ($total > 0) {
								$status = true;
							} else {
		...
		$method_data = array();
		if ($status) {  
			if ($this->config->get('payment_yandexplusplus_card_name_attach')) {
				$metname = htmlspecialchars_decode($this->config->get('payment_yandexplusplus_card_name_' . $this->config->get('config_language_id')));
			}
			else{
			...
			$method_data = array( 
				...
				'title'      => $metname,
				'terms'      => '',
				'sort_order' => $this->config->get('payment_yandexplusplus_card_sort_order')
			);
		}
            ...
            foreach ($instros as $instro) {
                if ($instro == 'checkouthref' || $instro == 'href' || $instro == 'orderid' || $instro == 'itogo' ||  $instro == 'itogobez' ||  $instro == 'itogozakaz' || $instro == 'komis' || $instro == 'total-komis' || $instro == 'plus-komis' || $instro == 'totals' || $instro == 'nds' || $instro == 'bvnds' || isset($order_info[$instro]) || substr_count($instro, "ordercustom_") || substr_count($instro, "shippingAddresscustom_") || substr_count($instro, "paymentAddresscustom_") || substr_count($instro, "customercustom_") || substr_count($instro, "paymentsimple4_") || substr_count($instro, "shippingsimple4_") || substr_count($instro, "simple4_")){
                    if ($instro == 'checkouthref'){
                        ...
                        $instro_other = $order_info['store_url'] . 'index.php?route=checkout/success_yandexplusplus_card&code=' . $this->getSecureCode($order_info['order_id']) . '&order='.$order_info['order_id'];
                    ...
                    if ($instro == 'total-komis'){
                        if($this->config->get('yandexplusplus_card_komis')){
                            $instro_other = $this->currency->format($out_summ * $this->config->get('yandexplusplus_card_komis')/100,  'RUB', $this->currency->getValue('RUB'), true);
                        }
                        else{$instro_other = '';}
                    }
                    if ($instro == 'plus-komis'){
                        if($this->config->get('yandexplusplus_card_komis')){
                            $instro_other = $this->currency->format($out_summ + ($out_summ * $this->config->get('yandexplusplus_card_komis')/100),  'RUB', $this->currency->getValue('RUB'), true);
                        }
                        else{$instro_other = '';}
                    }
                    if ($instro == 'totals'){
